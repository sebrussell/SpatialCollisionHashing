...
Graphics::Graphics()
{
	...
	if(TTF_Init() == -1)
	{
		printf("TTF could not initialise!");
	}
	else{
		...
		if(!arial)
		{
			...
			messageSurface = TTF_RenderText_Solid(arial, "This is a test of ttf", textColor);
		}
	}
}
...
SDL_Keycode Graphics::FirstUpdate(int _amountOfParticles)
{	
	...
	m_fpsStream.str(std::string());
	m_fpsStream << "Average FPS of: " << m_averageFPS << " with " << _amountOfParticles << " particles and " << m_tableSize  << " buckets.";
	messageSurface = TTF_RenderText_Solid(arial, m_fpsStream.str().c_str(), textColor);
	SDL_BlitSurface(messageSurface, NULL, screenSurface, NULL);
...
void Graphics::DrawLine(std::weak_ptr<Hash> _hashTable)
{
	...
	m_tableSize = tempHashTable->GetTableSize();
